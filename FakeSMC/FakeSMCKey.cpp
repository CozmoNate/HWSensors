...
{ 
	if (handler) {
        ...
        if (time - lastUpdated >= 1.0) {
            IOReturn result = handler->callPlatformFunction(kFakeSMCGetValueCallback, true, &key, &value, &size, 0);
...
bool FakeSMCKey::setValueFromBuffer(const void *aBuffer, UInt8 aSize)
{
	...
	if (handler) {       
		IOReturn result = handler->callPlatformFunction(kFakeSMCSetValueCallback, true, &key, &value, &size, 0);
