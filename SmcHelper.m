...
+ (BOOL)isValidFloatingSmcType:(NSString *)type
{
    if (type && [type length] >= 3) {
        if (([type characterAtIndex:0] == 'f' || [type characterAtIndex:0] == 's') && [type characterAtIndex:1] == 'p') {
            UInt8 i = [SmcHelper getIndexOfHexChar:[type characterAtIndex:2]];
            UInt8 f = [SmcHelper getIndexOfHexChar:[type characterAtIndex:3]];
    ...
    if (type || data) {
        ...
        if (typeLength >= 3) {
            if ((type[0] == 'u' || type[0] == 's') && type[1] == 'i') {
                ...
                UInt8 i = [SmcHelper getIndexOfHexChar:type[2]];
                UInt8 f = [SmcHelper getIndexOfHexChar:type[3]];
